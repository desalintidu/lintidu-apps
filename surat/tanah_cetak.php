...
if (isset($_POST["cetak"])) {
  ...
  $laporanTanah = mysqli_query($conn, "SELECT * FROM tb_surat_kepemilikan_tanah INNER JOIN tb_user ON tb_surat_kepemilikan_tanah.nik = tb_user.nik WHERE tanggal_surat_kepemilikan_tanah BETWEEN '$tglAwal' AND '$tglAkhir'");
} else {
  $laporanTanah = mysqli_query($conn, "SELECT * FROM tb_surat_kepemilikan_tanah");
}
...
<body>
  ...
  <div class="container">
      ...
      <table width="100%%" cellspacing="0">
        <tr>
          <td style="text-align: center; font-weight: bold; font-size: 16px;"><u>LAPORAN PENGAJUAN SURAT KEPEMILIKAN TANAH</u></td>
        </tr>
      </table>
      <!-- end nomor surat -->
...
$no = 1;
while ($row = mysqli_fetch_array($laporanTanah)) {
  $html .= "<tr style='text-align:center;'>
        ...
        <td>" . $row['pekerjaan'] . "</td>
        <td>" . $row['alamat'] . "</td>
        <td>" . $row['alamat_tanah'] . "</td>
        <td>" . date('d F Y', strtotime($row['tanggal_surat_kepemilikan_tanah'])) . "</td>
        </tr>";
  $no++;
}
...
$dompdf->stream('Laporan Pengajuan Surat Kepemilikan Tanah' . ".pdf", array("Attachment" => false));
