...
if (isset($_POST["cetak"])) {
  ...
  $laporanEkonomi = mysqli_query($conn, "SELECT * FROM tb_surat_ekonomi_lemah INNER JOIN tb_user ON tb_surat_ekonomi_lemah.nik = tb_user.nik WHERE tanggal_surat_ekonomi_lemah BETWEEN '$tglAwal' AND '$tglAkhir'");
} else {
  $laporanEkonomi = mysqli_query($conn, "SELECT * FROM tb_surat_ekonomi_lemah");
}
...
<body>
  ...
  <div class="container">
      ...
      <table width="100%%" cellspacing="0">
        <tr>
          <td style="text-align: center; font-weight: bold; font-size: 16px;"><u>LAPORAN PENGAJUAN SURAT EKONOMI LEMAH</u></td>
        </tr>
      </table>
      <!-- end nomor surat -->
...
$no = 1;
while ($row = mysqli_fetch_array($laporanEkonomi)) {
  $html .= "<tr style='text-align:center;'>
        ...
        <td>" . $row['nama_ayah'] . "</td>
        <td>" . $row['nama_ibu'] . "</td>
        <td>" . date('d F Y', strtotime($row['tanggal_surat_ekonomi_lemah'])) . "</td>
        </tr>";
  $no++;
}
...
$html .= '  <table cellspacing="0"  style="margin-top: 35px; margin-left: auto;">
      <tr>
          <td style="text-align: center; width: 250px;">Lintidu, <span>16 Desember 2022</span></td>
      </tr>
      <tr>
...
$dompdf->stream('Laporan Pengajuan Surat Ekonomi Lemah' . ".pdf", array("Attachment" => false));
