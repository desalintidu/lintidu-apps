...
if (isset($_POST["cetak"])) {
  ...
  $laporanEkonomi = mysqli_query($conn, "SELECT * FROM tb_surat_usaha INNER JOIN tb_user ON tb_surat_usaha.nik = tb_user.nik WHERE tanggal_surat_usaha BETWEEN '$tglAwal' AND '$tglAkhir'");
} else {
  $laporanEkonomi = mysqli_query($conn, "SELECT * FROM tb_surat_usaha");
}
$html = '
<html>
<head>
...
while ($row = mysqli_fetch_array($laporanEkonomi)) {
  $html .= "<tr style='text-align:center;'>
        ...
        <td>" . date('d F Y', strtotime($row['tanggal_surat_usaha'])) . "</td>
        </tr>";
  $no++;
}
